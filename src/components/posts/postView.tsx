          ...
          <div>
            <Link href={`/post/${item.postId}`}>
              <div>
                ...
                <span className="border-l ml-2 pl-3 text-post-text">
                  {useTimeRemaining(item.time)}
                </span>
              </div>
            </Link>
      ...
      <div className="flex justify-center pt-1">
        {data.map((item, i) => (
          <div
            className={`${
              i === 3
                ? "bg-pink-300 text-[20px] text-post-text py-1 px-2 rounded-full ml-2"
                : "bg-sky-100 text-[20px] text-post-text py-1 px-2 rounded-full ml-2"
            }`}
            key={i}
          >
            {i + 1}
